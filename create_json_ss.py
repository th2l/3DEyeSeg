...
def create_json_from_npy_data(dataset_filename, json_filename):
    ...
    if not os.path.isfile(os.path.join(dataset_filename, 'output.txt')):
        raise ("Label file list does not exist")

    with open(os.path.join(dataset_filename, 'output.txt'), 'r') as f:
        lines = f.readlines()
    ...
    for idx, l in enumerate(lines):
        ...
        cur_path = os.path.join(dataset_filename, l, 'pred.npy')
        if not os.path.isfile(cur_path):
            raise ("File does not exist: {}".format(cur_path))

        temp_label = np.load(cur_path)
        # save as uint8
        temp_label = temp_label.astype(np.uint8)
        np_b64_utf8_str = np_to_base64_utf8_str(temp_label)
        data['labels'][ntpath.basename(l.replace("/pred.npy", ""))] = np_b64_utf8_str
