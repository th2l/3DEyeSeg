...
def get_dataset_class(dataset_config):
    ...
    tp3d_dataset_module = ".".join(["torch_points3d.datasets", task, module])
    our_dataset_module = ".".join([".dataset", task, module])
    if importlib.util.find_spec(our_dataset_module, package=__package__) is not None:
        datasetlib = importlib.import_module(our_dataset_module, package=__package__)

    elif importlib.util.find_spec(tp3d_dataset_module) is not None:
        datasetlib = importlib.import_module(tp3d_dataset_module)
    else:
