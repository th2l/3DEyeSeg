...
def instantiate_model(config, dataset) -> BaseModel:
    ...
    tp3d_model_module = ".".join(["torch_points3d.models", task, module])
    our_model_module = ".".join([".models", task, module])

    if importlib.util.find_spec(our_model_module, package=__package__) is not None:
        modellib = importlib.import_module(our_model_module, package=__package__)

    elif importlib.util.find_spec(tp3d_model_module) is not None:
        modellib = importlib.import_module(tp3d_model_module)
    else:
        raise ValueError("Could not find {}.{}".format(task, module))
